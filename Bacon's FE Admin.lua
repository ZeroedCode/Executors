...
if (not UndetectedMode and not game.IsLoaded(game)) then
    print("bacons admin: waiting for game to load...");
...
if (game.IsLoaded(game) and UndetectedMode and syn) then
    syn.queue_on_teleport("loadstring(game.HttpGet(game, \"https://pastebin.com/raw/6s6yW3Qr\"))()");
...
if (getgenv().F_A and getgenv().F_A.Loaded) then
    return getgenv().F_A.Utils.Notify(nil, "Loaded", "bacons admin is already loaded... use 'killscript' to kill", nil);
end
...
AddCommand("commandcount", {"cc"}, "shows you how many commands there is in bacons admin", {}, function(Caller)
...
AddCommand("advertise", {}, "advertises the script", {}, function()
    local ChatRemote = Services.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
    ChatRemote.FireServer(ChatRemote, "I am using bacons admin, join the server 5epGRYR", "All");
end)

AddCommand("joinserver", {"discord"}, "joins the bacons admin discord server", {}, function()
    ...
    if (Request({
        ...
        return "joined bacons admin discord server"
    else
...
AddCommand("changelogs", {"cl"}, "shows you the updates on bacons admin", {}, function()
...
AddCommand("commands", {"cmds"}, "shows you all the commands listed in bacons admin", {}, function()
...
AddConnection(CConnect(ChatLogs.Save.MouseButton1Click, function()
    ...
    local String =  format("bacons Admin Chatlogs for %s (%s)\n\n", GameName, os.date());
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Name = format("bacons-admin/chatlogs/%s (%s).txt", GameName, TimeSaved);
    for i, v in next, GetChildren(ChatLogs.Frame.List) do
        if (not IsA(v, "UIListLayout")) then
            String = format("%s%s\n", String, v.Text);
        end
...
Utils.Notify(LocalPlayer, "Welcome To Bacons Admin", "'cmds' to see all of the commands");
local LatestCommit = JSONDecode(Services.HttpService, game.HttpGetAsync(game, "https://pastebin.com/raw/PTxjpZBm"))[1]
wait(1);
